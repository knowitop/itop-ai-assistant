...
- A few semantic fields hold a **list** of values rather than one — today that is `customer_org_ids` on FAQ, the customer organizations an article is published to. Their row takes `<link set>:<id attribute of a link>` (e.g. `customers_list:customer_id`) instead of a plain attribute code; the row's own description says so. Writing that form on a row that is not list-valued is refused when you save — the halves are told apart by the field's declaration, not by the colon, so it would reach iTop whole and break the read of the whole class. For a field you added yourself, **Several values** in its form is what makes it list-valued. See [List-valued fields and who may see an object](configuration.md#list-valued-fields-and-who-may-see-an-object)
...
Each class also says **which fields give access to it** — the semantic fields whose values name the organizations allowed to see an object. A search narrows its candidates to objects sharing an organization with whoever is asking before the vector walk starts; an object naming none is left to the rights check under that person's own token. The picker offers what the source has (tickets: `org_id`; FAQ: `org_id` and `customer_org_ids`, both mapped in **Data model mapping**), and a name it does not know is refused when you save. Changing this only reaches objects already indexed on a full pass, so the tab warns and offers to start one — it re-embeds nothing, only the stored metadata is rewritten.
