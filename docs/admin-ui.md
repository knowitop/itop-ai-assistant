...
**Settings tab** — whether indexing is on (applies from the next sweep, no restart), the system-wide sweep interval / page size / throttle, the reconciliation interval, and chunk token budget, followed by one section per **family** (one Qdrant collection each — today: **tickets**, **FAQ**). A family section holds its own optional overrides for the sweep interval and the log-entries-per-chunk window (blank = use the system-wide value above — useful for a source like FAQ, which has no incremental cursor and re-scans its whole class on every sweep), and its classes: which values of each class's relevance attribute keep an object in the index (empty = index everything), and what each fragment of an indexed object contains. Adding a class to a family's list is enough to index it under that family — no code change, as long as the family's source can read the class.
