...
- **Fields you added** — semantic fields the assistant has no idea about, declared here and then mapped in the table above like any other, e.g. `{"vendor_id": {"kind": "id", "roles": ["organization"]}}`. `kind` is `text`, `id` or `enum` (`"multi": true` for a link set), and the roles are what the field is *for*: `organization` lets it grant access to the object, `content` lets it feed a chunk fragment. A name the family already has is rejected. An `id` or `enum` value is stored alongside the object in the index — the only way a field nothing in the code reads is of any use — and reaches objects already indexed only on a full pass
