...
Fine-grained editing of every connection section after initial setup — iTop, LLM, embeddings and security. Changes apply from the next processed ticket without a restart, and each section can be reset to its environment defaults.
...
Only needed for the optional [vector index](#vector-index). Leave it empty and intake works as it always has, with one thing missing: the handoff note carries no references to similar solved tickets or relevant FAQ articles.
...
How our semantics map onto your iTop datamodel — edit this instead of the code when iTop has been customized. One tab per **object family** (`tickets`, `faq`), and inside it two tables: **Fields the assistant declares** and **Fields added here**. A row is one semantic field: the iTop attribute code on the right, and a **No such attribute** switch for what your datamodel does not have.

- The families and their rows come from the assistant's own declarations, so the form always shows every field it can use, and a new family appears as a tab of its own without a UI change
- Every row carries what the field **is**, as the declaration has it: its kind (`text`, `id`, `enum`, `datetime`, `log`), **several values** where one mapping value yields several iTop values, and its roles (`content`, `organization`, `lifecycle_state`, …) — what the assistant will do with whatever the attribute you name returns. They are the identifiers the declaration and the API use, shown as they are
- The placeholder on an empty row is what the declaration maps by default; a row you never touch keeps that, and the switch stores `null`. The switch says what the row *means*, not what it holds: the attribute code stays in the input, greyed out, and is there again when you switch it off — until the family is saved, after which an absent row comes back showing the declaration's code
- A row of the first table that no longer matches its declaration gets a **Revert** button in the last column — one control for both halves of the same fact: it is there only while the row differs from what the code declares, and pressing it puts the row back. Staged like every other edit here, so it reaches the section when you save the family. A field you added yourself never shows it: it is its own baseline
...
- **Save** belongs to the tab it sits in and sends that family alone (`PATCH /api/setup/mappings/families/{family}`). The families are one config section, but the merge that keeps the others is the server's and reads them as they stand — so nothing you did not edit is rewritten, and two people editing two families do not overwrite each other. Edits open in another tab are neither saved by this button nor lost; that tab saves its own
...
- **Add a field** — a semantic field the assistant has no idea about. The button under the table opens a form: what the field **is** (`text`, `id` or `enum`, one value or a link set), what it **means** in the object, a description, and the iTop attribute code it reads — required, because a field of your own has no default to fall back on and reads nothing until it names one. Unmapping it later is still possible from its own row, where it is a deliberate act rather than an unfinished declaration. A role the chosen kind cannot carry, or one the family already has a field for, is offered greyed out with the reason — `lifecycle_state` says which field carries it already. `organization` lets the field grant access to the object, `content` lets it feed a chunk fragment, and a field with no role is carried into the index and nothing else. The field then takes a row in **Fields added here**, with the edit and remove icons beside it; removing it takes its attribute code and any per-class override of it along. A name the family already has is refused. An `id` or `enum` value is stored alongside the object in the index — the only way a field nothing in the code reads is of any use — and reaches objects already indexed only on a full pass
- A semantic field can hold a **list** of values rather than one — the customer organizations an article is published to, say. No field the assistant declares is like that; **Several values** in the form of a field you add yourself is what makes it list-valued. Such a row takes `<link set>:<id attribute of a link>` (e.g. `customers_list:customer_id`) instead of a plain attribute code. Writing that form on a row that is not list-valued is refused when you save — the halves are told apart by the field's declaration, not by the colon, so it would reach iTop whole and break the read of the whole class. See [List-valued fields and who may see an object](configuration.md#list-valued-fields-and-who-may-see-an-object)
- **Reset to defaults**, beside Save, puts **that** family back to what the code and env/yaml say — its fields, its per-class overrides and any field added to it. The other families stay as they are
...
Optional — the screen only does something when `QDRANT_URL` points at a Qdrant instance. One thing reads the index today: intake, for the similar solved tickets and relevant FAQ articles it quotes in the handoff note (see [intake settings](configuration.md#intake-module-settings)). What else could be built on it waits for features still to come.

**Status tab** — badges for the vector store, embeddings and indexer state; the active index version with row count, with a family switched off on the Indexed classes tab marked "indexing off" (its collection is still there, it is simply no longer updated); the per-class sweep cursors and the last reconciliation; and a table of recent indexing runs (objects seen, objects skipped, chunks embedded, chunks with metadata refreshed, chunks deleted, duration). "Chunks with metadata refreshed" counts chunks whose text was unchanged but whose status/org/filters were rewritten without a re-embed. "Objects skipped" counts objects the pass deliberately left out — an object chunking past **Max chunks per object** on the Indexer tab; which object and why is spelled out under the run's row, in grey rather than red, because the run itself succeeded. **Index now** runs the next ordinary pass immediately instead of waiting out the sweep interval: only objects changed since the last pass are re-embedded, which is what makes it cheap enough to press after editing the settings. **Reindex** schedules a full rebuild — every object is re-embedded, so it can take a while and load the embeddings endpoint. Changing the embeddings model needs neither this button nor anything else: vectors from two models cannot be compared, so the sweep builds a new index version next to the live one and switches over when it is complete. While that is running the family's row shows both sides of the move (`v1 → v2`, old model → new, and the new version's growing row count), and searches over that family are refused rather than answered from an index the query no longer matches — intake simply stops quoting similar tickets. Putting the previous model back before the new version is finished restores search immediately, since the old index was never touched. The one case that does need a hand: a family switched off on the Indexed classes tab is not swept, so its index is never rebuilt — the warning then names it, and switching the family back on starts the rebuild.
...
**Indexed classes tab** — one section per **family** (one Qdrant collection each — today: **tickets**, **FAQ**). A family section starts with **Index this family**: switch it off and the sweep skips that corpus whole, reconciliation leaves it alone and searches over it are refused — index the FAQ articles without the ticket archive, or the other way round, instead of choosing between both and neither. The collection is kept, so switching it back on resumes where it stopped. Below that sit the family's own optional overrides for the sweep interval and the log-entries-per-chunk window (blank = use the system-wide value from the Indexer tab — useful for a source like FAQ, which has no incremental cursor and re-scans its whole class on every sweep), and its classes: which values of each class's relevance attribute keep an object in the index (empty = index everything), and what each fragment of an indexed object contains. Adding a class to a family's list is enough to index it under that family — no code change, as long as the family's source can read the class. Naming relevance values requires the family's lifecycle-state field (`status`) to be mapped in **Data model mapping**: without it there is nothing to compare them against, so the sweep refuses the class and says so in **Recent runs** instead of treating every object as irrelevant and dropping the class from the index.
...
An object is indexed as several **fragments**, matched separately — a query can look like another ticket's description without looking like its solution, and keeping them apart is what makes the difference visible. Which fragments exist is decided by the indexing source, not by you: tickets have `profile`, `body`, `solution` and the two case logs. For each of the first three you pick the semantic fields that feed it (title, description, solution, service_name, subcategory_name) — that is where you adapt to a customized iTop datamodel. A fragment with no fields selected produces nothing, and the screen warns when a class ends up producing nothing at all.

Each class also says **which fields give access to it** — the semantic fields whose values name the organizations allowed to see an object. A search narrows its candidates to objects sharing an organization with whoever is asking before the vector walk starts; an object naming none is left to the rights check under that person's own token. The picker offers what the source has — `org_id` for both families, plus any field you declared with the `organization` role, all mapped in **Data model mapping** — and a name it does not know is refused when you save. Changing this only reaches objects already indexed on a full pass, so the tab warns and offers to start one — it re-embeds nothing, only the stored metadata is rewritten.
