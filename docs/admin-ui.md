...
- **Multi-valued fields** — for a semantic field whose value is a list rather than one attribute: several attributes read together, or an n-n link set. One row per value: the semantic name, whether it is a plain **Attribute** or a **Link set**, the attribute code, and — for a link set — the attribute of the *link* that carries the id. Rows sharing a semantic name are merged into one list, and a row with no name or no attribute is dropped on save. This is where a build that publishes FAQ articles to a list of customer organizations maps that list; see [Multi-valued fields and who may see an object](configuration.md#multi-valued-fields-and-who-may-see-an-object)
...
Each class also says **which fields give access to it** — the semantic fields whose values name the organizations allowed to see an object. A search narrows its candidates to objects sharing an organization with whoever is asking before the vector walk starts; an object naming none is left to the rights check under that person's own token. The picker offers what the source has (tickets: `org_id`, `provider_id`; FAQ: `org_id`, `customer_org_ids`, both mapped in **Data model mapping**), and a name it does not know is refused when you save. Changing this only reaches objects already indexed on a full pass, so the tab warns and offers to start one — it re-embeds nothing, only the stored metadata is rewritten.
