...
| `EMBEDDINGS_BASE_URL` / `EMBEDDINGS_MODEL` / `EMBEDDINGS_API_KEY` | optional | OpenAI-compatible `/v1/embeddings` endpoint for the vector index. Must include `/v1` — for Ollama that's `http://localhost:11434/v1`, not the bare `host:port` used for the LLM chat connection |
