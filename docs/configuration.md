...
A background sweep picks up objects changed since the last pass, splits each one into the fragments configured for its class (`vector.families.<family>.classes.<class>.chunks` — see [Admin UI → Vector index](admin-ui.md#vector-index)), and embeds only what actually changed (content-hash guard). It runs every `sweep_interval_seconds`, and that interval is counted **from the last pass, not from the last restart**: restarting the service — a container restart, an image update — does not start a pass of its own, and an installation restarted often is indexed exactly as often as one that is not. What still runs immediately is what you ask for by hand: **Index now** and **Reindex** on the Vector page, and the `itop-ai-reindex` CLI. The index stores **embeddings, ids and filter metadata only — never ticket or article text**; anything shown to a user is re-fetched fresh from iTop, so the index is a rebuildable cache, not a copy of your ticket database.

An object that splits into more than `max_chunks_per_object` chunks (32 by default — roughly a 46 000-character document at the default chunk size) is **skipped whole**, before anything is sent to the embeddings endpoint. The pass counts it under "objects skipped" and names it under its row in the run history; the run itself still succeeds, because skipping junk is what the limit is for. An object that large is practically always an attachment inlined into the body rather than a long article, and embedding it costs real money on a paid endpoint. Whatever that object already has in the index stays there; raise the limit if your deployment genuinely has documents that long.

FAQ fields (`title`, `summary`, `category_name`, `error_code`, `key_words`, `description`) map onto your iTop datamodel through the `mappings` config section, under the `faq` family — one section covers every family, and all of them are edited in the same place: [Admin UI → Connections → Data model mapping](admin-ui.md#data-model-mapping-tab). The section holds only what you changed: a field you never touch is mapped as the assistant declares it, so a field added by a later release needs no edit here.

You can also **add fields of your own** to a family — `mappings.families.<family>.declared` — for a datamodel that carries something the assistant does not know about. Such a field is declared (`kind`: `text`, `id` or `enum`, optionally `multi`, `roles`, a description) and then mapped in the same table as everything else — [Admin UI → Connections → Data model mapping](admin-ui.md#data-model-mapping-tab) has a form for it, so this key rarely needs writing by hand. `roles` is what it is for: `organization` makes it one of the fields that may be named in `acl_org_fields`, `content` makes it composable into a chunk fragment. A field with no role is simply stored alongside the object in the index, where a later filter can find it — which is the only way a field nothing in the code reads by name is of any use. Text is never stored there, only `id` and `enum` values. A name the family already has is refused; a field left without an attribute code is not — it has no default to fall back on, so it reads nothing, and a line in the log at startup names it. A value added this way reaches objects indexed earlier only after a full pass (`POST /api/vector/reindex` — no re-embedding). Every FAQ field is optional: the defaults follow stock iTop, and a customized `FAQ` class need not carry all of them — `error_code`, `summary` and `key_words` are the usual ones to go missing. **An attribute that is mapped but does not exist in your iTop is not ignored** — the repository asks iTop for it and the whole sweep of the FAQ family fails, so switch off what your datamodel lacks. `status`, `org_id` and both dates are unmapped by default — stock iTop's `FAQ` carries none of them — map the ones your deployment's `FAQ` actually has.

Changing the embeddings model or its dimension invalidates every stored vector — vectors from different models are not comparable. The assistant never mixes them and rebuilds instead, by itself: the next sweep pass starts a new index version alongside the live one, fills it from scratch, and switches over once it is complete. Nothing has to be pressed, and nothing has to be deleted by hand. Expect it to take as long as the original indexing did — every object is embedded again — and expect searches over that family to be refused until it finishes, since the old index cannot answer a query embedded by the new model. The old index is kept whole until the switch, so putting the previous model back in the config restores search at once instead of costing a second rebuild. Progress is on the Vector page (**Status** tab): the family's row shows `v1 → v2` and the new version's row count going up. A family switched off (`vector.families.<family>.enabled`) is not swept and so is not rebuilt either — switch it back on to start.

### List-valued fields and who may see an object

Some facts are not one attribute. If your build publishes a knowledge base article to a **list** of customer organizations, that value is an n-n link set rather than an attribute code. A few semantic fields are therefore declared as lists — the `faq` family's `customer_org_ids` is the one that exists today — and their row in the mapping table takes a value of the form `<link set>:<id attribute of a link>`, e.g. `customers_list:customer_id`. The half after the colon is the attribute of the **link** that carries the id, not an attribute of the organization itself. Everything else about the row is ordinary: it lives in the same table as every other field, mapped in [Admin UI → Connections → Data model mapping](admin-ui.md#data-model-mapping-tab), and it is unmapped by default because stock iTop has no such list. A plain attribute code works there too and yields the one value it holds — what makes the field a list is the declaration, not the mapping. The other way round is refused when you save: nothing tells the two halves apart by the colon, so on a field that holds one value the whole string would reach iTop's `output_fields` and fail the read of every object of that class.

What that buys is the **organization pre-filter**. A search narrows its candidates to objects that share an organization with whoever is asking (their *Allowed Organizations* in iTop) before the vector walk starts; whether they may actually see a candidate is then decided by iTop itself, under that person's own token. Which semantic fields say who may see an object of a class is configuration — `vector.families.<family>.classes.<class>.acl_org_fields`, chosen in [Admin UI → Vector index](admin-ui.md#vector-index) from what the source offers (tickets: `org_id`; FAQ: `org_id` and `customer_org_ids`). A name outside that list is refused when the section is saved. The default is `org_id` for tickets and nothing for FAQ, matching stock iTop.

Two things to know before you rely on it:

- **An object that names no organization is passed through, not hidden.** The index has nothing to narrow by — in iTop a missing organization means either that the silo does not apply to the class or that the right is inherited from an object the index knows nothing about — so the decision is left to the rights check under the asking person's own token. This is why a mis-mapped attribute does not hide anything: it quietly switches the pre-filter off for that class instead. The sweep counts objects with no organization per class and warns in the run history when a class that *declares* `acl_org_fields` comes back almost entirely empty — that warning is the thing to watch after changing this mapping.
- **A change reaches already-indexed objects only on a full pass.** Objects keep the organizations they were indexed with until the sweep visits them again, and changing an n-n link set in iTop does not necessarily move the object's modification date, so the incremental sweep may never notice. Run **Reindex** (or `itop-ai-reindex --full`) after editing anything about access. It costs no embeddings — the text has not changed, only the payload is rewritten.
