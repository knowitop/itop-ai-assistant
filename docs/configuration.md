...
An object that splits into more than `max_chunks_per_object` chunks (32 by default — roughly a 46 000-character document at the default chunk size) is **skipped whole**, before anything is sent to the embeddings endpoint, and reported as an error for that pass. An object that large is practically always an attachment inlined into the body rather than a long article, and embedding it costs real money on a paid endpoint. Whatever that object already has in the index stays there; raise the limit if your deployment genuinely has documents that long.
