...
FAQ fields (`title`, `summary`, `category_name`, `error_code`, `key_words`, `description`) map onto your iTop datamodel through the `faq_mapping` config section — same idea as `ticket_mapping`, and both are edited in the same place: [Admin UI → Connections → Data model mapping](admin-ui.md#data-model-mapping-tab). Every FAQ field is optional: the defaults follow stock iTop, and a customized `FAQ` class need not carry all of them — `error_code`, `summary` and `key_words` are the usual ones to go missing. **An attribute that is mapped but does not exist in your iTop is not ignored** — the repository asks iTop for it and the whole sweep of the FAQ family fails, so switch off what your datamodel lacks. `status`, `org_id` and both dates are unmapped by default — stock iTop's `FAQ` carries none of them — map the ones your deployment's `FAQ` actually has.
