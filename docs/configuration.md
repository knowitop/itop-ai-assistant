...
Two independent sources are indexed into their own collections today: **tickets** (`UserRequest`, `Incident` — the "similar past tickets" scenario) and **FAQ** (the "relevant knowledge base article" scenario, class `FAQ`). Each source gets its own Qdrant collection and its own HNSW graph — the two scenarios are never searched together in one call. Stock iTop's `FAQ` class has neither a lifecycle status nor a date attribute, so by default every article is indexed and every sweep pass re-reads the whole class (cheap: unchanged articles are neither re-embedded nor rewritten, only re-read) — map `status`/`last_update` in `faq_mapping` if your deployment's `FAQ` does carry either.
