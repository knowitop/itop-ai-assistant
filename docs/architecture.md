...
**8.2 Ввести принципала прогона.** Сегодня личность прогона подразумевается: провайдер клиента отдаёт один бандл, собранный из секции `itop`, и нигде не записано, что все обращения идут от сервисной учётки. Нужно: тип принципала в ядре (`service` / делегированный), объявление режима модулем, принципал в контексте прогона и в журнале, атрибуция действий через `comment`.
