...
**8.4 Разместить каждый инвариант на том шве, свойством которого он является.** Деление на «платформенные» и «модульные» — неверная ось: она сортирует правила по важности, а важность не говорит, где их исполнять. Рабочий критерий другой: **если модуль может обоснованно хотеть иначе — это его политика, и место ей в модуле.** Образец настоящего инварианта ядра уже лежит в оболочке — блокировка объекта (`pipelines/shell.py`): она стоит в `execute()` безусловно, модулю не делегируется, и модуля, которому законно нужна параллельная обработка одного объекта, не существует.

По этому критерию три проверки, которые intake делает сегодня, расходятся на три разных шва — и ни одна не попадает в оболочку:

| Правило | Свойство чего | Где исполнять |
|---|---|---|
| Не писать, когда инженер взял объект | Политики intake | `IntakeRun.stop_reason` — где и лежит |
| Не реагировать на собственный комментарий | Пары «вебхук по объекту + запись в него же» | Вебхучный маршрут пишущего модуля |
| Не более одной записи в публичный лог за прогон | Канала | Шов записи: бюджет записей на прогон |

Первое — не платформа ни в каком виде: у консоли условие ровно обратное, она нужна именно на взятом тикете. Правило, которое модуль вправе выключить, инвариантом ядра не является — перенос в оболочку означал бы флаг «мне это не надо», то есть ту же политику, только дальше от модуля.

Второе — защита от петли, а петля бывает не у всякого триггера: консоль запускает инженер запросом, и от собственной записи она не перезапускается; у `schedule` инициатор — часы. Проверка вдобавок приклеена к сервисному аккаунту: сравнение идёт с `ai_person_name()`, который намеренно резолвится по **сервисному** бандлу независимо от принципала прогона (§3.5). Под делегированным токеном последняя запись была бы за именем инженера, и то же сравнение означало бы уже не то же самое.

Третье — единственное, что переживает проверку как общее правило, но переезжать ему не в оболочку. Публичный лог — это входящие заявителя, и не заваливать их очередью сообщений обязан любой пишущий туда модуль, объектный он или нет. Оболочка про **объект** (§3.2), и прогон без объекта через неё не проходит вовсе — `selfcheck` уже такой. Значит, шов — сам путь записи, а не guard.

Опасность, которой это предложение оправдывалось изначально — в публичном логе появляется второй голос, и никто не замечает, — реальна. Но аргументом она служит только за третью строку таблицы, и именно на шве записи её и надо закрывать.
