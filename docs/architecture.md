...
| Хранение делегированных токенов | Выдача и отзыв токенов (iTop) |
...
Все три реализованы и идут через один реестр, одну рамку и один журнал — тип триггера записан в прогоне и виден на экране Runs. У `request` и `schedule` пока нет принципала: они работают от сервисной учётки модуля, токен инженера придёт с §8.2.
...
Разрешение принципала (первая строка схемы выше) в ядро пока не вынесено — это §8.2.
...
Делегирование реализуется хранением персонального токена iTop, который инженер вводит один раз. Следствия, которые надо принять целиком:

- **Middleware становится хранилищем секретов пользователей.** Токены шифруются ключом развёртывания и живут рядом с рантайм-конфигом. Компрометация хранилища = компрометация всех выданных токенов, поэтому ключ не должен лежать там же, где шифротекст.
- **Нужен жизненный цикл**: ввод при первом обращении, проверка живости, понятная ошибка при отзыве, удаление по инициативе инженера и по уходу из компании.
...
| Redis | Состояние обработки объекта, блокировки, оверрайды конфига и промптов, журнал прогонов, делегированные токены | да |
...
С появлением делегированных прав журнал обязан фиксировать **принципала прогона**: чтение под токеном инженера в iTop почти не оставляет следов, и единственным местом, где видно «кто именно спрашивал», становится наш журнал.
...
sequenceDiagram
    ...
    WG->>P: запрос с идентификатором инженера и тикета
    P->>P: соединение из делегированного токена
...
| Консоль | вход | Идентификатор инженера, доступ — его делегированный токен |
| Управление своим токеном | вход | Сам инженер |
...
**8.2 Ввести режимы доступа и именованные соединения.** Провайдер клиента кэширует один бандл по отпечатку одной секции. Нужны: именованные системные соединения и соединения, собираемые из делегированного токена по принципалу. Кэш становится ключом по паре «принципал + отпечаток» с вытеснением и закрытием клиентов.
...
Принципала у request и schedule пока нет: прогон идёт от сервисной учётки, эндпоинт закрыт админским токеном (см. §8.2).
...
**8.5 Хранилище делегированных токенов.** Шифрование ключом развёртывания, ключ не рядом с шифротекстом, экран управления своим токеном, понятная ошибка при отзыве. Это первый секрет пользователя в системе — обращаться с ним как с конфигом нельзя.

**8.6 Принципал в журнале прогонов.** Чтение под токеном инженера не оставляет следов в iTop; наш журнал становится единственным местом, где видно, кто и о чём спрашивал.
...
| Делегированный токен инженера | Middleware хранит секреты пользователей | Права наследуются от iTop; свою модель доступа писать не нужно |
...
- Токен на инженера → OAuth-подобная схема, если появится отзыв и ротация на стороне iTop.
...
| Журнал прогонов и экран Runs | реализовано: тип триггера и субъект есть, принципала нет |
...
| Режимы доступа и делегированные токены | предложено, §8 |
