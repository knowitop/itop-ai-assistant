...
def _with_customer_orgs() -> Schema:
    ...
    values, so a list-valued field is one an administrator added."""
    return FAQ_SCHEMA.extended(
        [
            FieldSpec(
                "customer_orgs",
                FieldKind.ID,
...
class TestSchema(unittest.TestCase):
    ...
    def test_several_organizations_may_grant_access(self):
        ...
        self.assertEqual(("org_id", "customer_orgs"), _with_customer_orgs().names(Role.ORGANIZATION))
    ...
    def test_multi_is_declared_not_inferred(self):
        self.assertEqual(frozenset({"customer_orgs"}), _with_customer_orgs().multi_names())
        self.assertEqual(frozenset(), FAQ_SCHEMA.multi_names())
    ...
    def test_a_link_set_mapping_needs_a_field_that_holds_several_values(self):
        ...
        _with_customer_orgs().check_mapping({"customer_orgs": "customers_list:customer_id"}, by="a deployment")
